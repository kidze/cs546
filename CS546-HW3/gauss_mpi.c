...
void gauss_mpi() {
	...
	for (norm = 0; norm < N - 1; norm++) {
		...
		if(myid == 0){
			...
			for(proc = 1; proc<p ; proc++){
				for(row = norm + 1 + proc; row < N; row+=p){
					MPI_Irecv(&A[row], N, MPI_DOUBLE, proc, 1, MPI_COMM_WORLD, &request);
					MPI_Irecv(&B[row], 1, MPI_DOUBLE, proc, 1, MPI_COMM_WORLD, &request);
				}
			}
		...
		//other processors
			...
			for(row = norm + 1 + myid; row < N; row+=p){
				...
				MPI_Irecv(&A[row], N, MPI_DOUBLE, 0, 0, MPI_COMM_WORLD, &request);
				MPI_Irecv(&B[row], 1, MPI_DOUBLE, 0, 0, MPI_COMM_WORLD, &request);
				multiplier = A[row][norm] / A[norm][norm];
				for (col = norm; col < N; col++) {
					A[row][col] -= A[norm][col] * multiplier;
				}
